...
export class QuizService {
  ...
  generateQuiz(req: QuizGenerationRequest): Observable<GeneratedQuizDto> {
    ...
    return this.http.post<GeneratedQuizDto>(
  ...
  getQuizById(id: number): Observable<GeneratedQuizDto> {
    ...
    return this.http.get<GeneratedQuizDto>(
      `${this.serviceUrl}/${id}`
    );
  ...
  getAllQuizzes(
    page: number = 0,
    size: number = 10
  ): Observable<Page<QuizResponseDto>> {

    return this.http.get<Page<QuizResponseDto>>(
      `${this.serviceUrl}?page=${page}&size=${size}`
    );
